...
namespace stan {
  namespace math {
     ...
     * <code>categorical_rng(softmax(...))</code>. Because this function
     * recomputes the softmax for beta internally at every call, manually
     * calling the softmax, storing this as a temporary, and then
     * making calls to <code>categorical_rng(softmax_tmp)</code> as
     * needed will be significantly more efficient.
     *
     * @tparam RNG Type of pseudo-random number generator.
     * @param beta Vector of unnormalized log probabilities.
     * @param rng Pseudo-random number generator.
     * @return Integer in range [1, beta.size()]
     */
    template <class RNG>
    inline int
